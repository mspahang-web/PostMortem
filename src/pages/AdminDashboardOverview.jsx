...
function getSection7Issues(report) {
...
// =========================================================
// SECTION 11 - CADANGAN PROGRAM SUKMA 2028
// =========================================================

const SUKMA_2028_ITEMS = [
  {
    key: 'atletDikekalkan',
...
function getSection11Data(report) {
  ...
  if (
    !section11 ||
    typeof section11 !== 'object' ||
    Array.isArray(section11)
  ) {
    return {}
  }
...
function hasSection11Value(value) {
  if (Array.isArray(value)) {
    return value.some(
      (item) =>
        String(item ?? '').trim() !== ''
...
function calculateSukma2028Analytics(
  reports
) {
  const componentStats =
    SUKMA_2028_ITEMS.map((item) => {
      const filledReports =
        reports.filter((report) => {
          const section11 =
            getSection11Data(report)

          return hasSection11Value(
            section11[item.key]
          )
        }).length
  ...
  const reportsWithSection11 =
    reports.filter((report) => {
      const section11 =
        getSection11Data(report)

      return Object.values(section11).some(
        hasSection11Value
      )
    }).length
  ...
  const totalComponents =
    reports.length *
    SUKMA_2028_ITEMS.length

  const filledComponents =
    componentStats.reduce(
      (total, item) =>
        total + item.filledReports,
      0
    )

  return {
    totalReports: reports.length,
    reportsWithSection11,
    componentStats,
    filledComponents,
    totalComponents,
...
// =========================================================
// SECTION 12 - RUMUSAN PASUKAN
// =========================================================

function getSection12Data(report) {
  const section12 = report?.section_12

  if (
    !section12 ||
    typeof section12 !== 'object' ||
    Array.isArray(section12)
  ) {
    return {}
  }
...
function getSection12Text(
  section12,
  key
) {
  const value = section12?.[key]
...
function calculateRumusanAnalytics(
  reports
) {
  const records = reports.map(
    (report) =>
      getSection12Data(report)
  )

  const improvements = records.flatMap(
    (section12) =>
      getSection12Text(
        section12,
        'perkaraUtamaPerluDiperbaiki'
      )
  )

  const support = records.flatMap(
    (section12) =>
      getSection12Text(
        section12,
        'bentukSokongan'
  ...
  const sukma2028 = records.flatMap(
    (section12) =>
      getSection12Text(
        section12,
  ...
  return {
    ...
    reportsWithRumusan:
      records.filter(
        (section12) =>
          Object.keys(section12).length > 0
      ).length,
...
function calculateIssueAnalytics(
  reports
) {
...
function IssueAnalytics({
  ...
  if (total === 0) {
    return (
      <EmptyChart
        icon="!"
        title="Belum ada rekod isu"
        description="Analisis akan dipaparkan selepas isu teknikal pertandingan direkodkan."
      />
    )
  }
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        >
          KATEGORI / ACARA YANG DIREKODKAN
        </div>
      ...
      >
        ...
        rekod isu yang diisi dalam
        Bahagian G setiap laporan
        post-mortem.
      </div>
...
function AdminDashboardOverview({
  ...
  const sukma2028Analytics =
    useMemo(
      () =>
        calculateSukma2028Analytics(
          filteredReports
        ),
      [filteredReports]
    )
  ...
  const rumusanAnalytics =
    useMemo(
      () =>
        calculateRumusanAnalytics(
          filteredReports
        ),
      [filteredReports]
    )
    ...
    <div className="ewcc-dashboard-overview">
      ...
      <div className="ewcc-dashboard-chart-grid">
        ...
        <DashboardPanel
          ...
          description="Analisis rekod isu daripada Bahagian G."
        >
...
<DashboardPanel
  ...
  description="Status pengisian cadangan persediaan daripada Bahagian K."
  className="large"
>
  {loadingAnalytics ? (
    <EmptyChart
      icon="◌"
      title="Memuatkan analitik"
      description="Data cadangan SUKMA 2028 sedang diproses."
    />
  ) : sukma2028Analytics.totalReports === 0 ? (
    <EmptyChart
      icon="◎"
      title="Belum ada laporan"
      description="Cadangan SUKMA 2028 akan dipaparkan selepas laporan post-mortem tersedia."
    />
  ) : (
    <div
      style={{
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {sukma2028Analytics.totalReports}
          </strong>
        </div>
      ...
      >
        ...
        >
          <div>
            ...
            >
              Data ini akan menjadi asas
              kepada modul Cadangan
              Peralatan 2028.
            </span>
          </div>
      ...
      >
        Analisis dikira berdasarkan
        pengisian Bahagian K daripada
        laporan post-mortem yang telah
        dimuatkan.
      </div>
    </div>
  )}
...
<DashboardPanel
  ...
  description="Ringkasan perkara utama, keperluan sokongan dan cadangan SUKMA 2028 daripada Bahagian L."
  className="large"
>
  {loadingAnalytics ? (
    <EmptyChart
      icon="◌"
      title="Memuatkan rumusan"
      description="Data rumusan pasukan sedang diproses."
    />
  ) : rumusanAnalytics.reportsWithRumusan === 0 ? (
    <EmptyChart
      icon="◎"
      title="Belum ada rumusan"
      description="Rumusan pasukan akan dipaparkan selepas laporan dilengkapkan."
    />
  ) : (
    <div
      style={{
